...
class JITDocsTest < Minitest::Test
  ...
  def test_transport_export_topic_locks_runtime_export_commands
    ...
    assert_includes data[:rules], "Transport JSON is the canonical Ruby-to-backend handoff."
    assert_includes data[:rules], "Use `production_export SOURCE.rb OUT_DIR --stem STEM` when writing JSON, MusicXML, and MIDI."
