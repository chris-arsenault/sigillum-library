  ...
  module OrchestralDSL
    module Production
      ...
      def parse_bar_range(value)
        ...
        text = value.to_s
        if text.include?(",")
          bars = text.split(",").flat_map do |piece|
            m = piece.strip.match(/\A(\d+)(?:-(\d+))?\z/)
            raise ArgumentError, "bar list must look like 2,4,8-10" unless m

            (Integer(m[1])..Integer(m[2] || m[1])).to_a
          end
          return bars.uniq.sort
        end
      ...
      def degree_to_midi(token, tonic_midi)
        ...
        degree = degree_text.to_i
        accidental_offset = accidentals.chars.sum { |char| char == "#" ? 1 : -1 }
        octave_offset = (ups.length - downs.length) * 12
        tonic_midi + MAJOR_STEPS[degree - 1] + accidental_offset + octave_offset
      end
      ...
      def degree_to_pitch(token, tonic_name, tonic_midi)
        ...
        _accidentals, degree_text, _ups, _downs = match.captures
        ...
        target_index = tonic_letter_index + degree - 1
        letter = LETTERS[target_index % 7]
        letter_octave = tonic_octave + (target_index / 7)
        letter_octave += token.count("'")
        letter_octave -= token.count(",")
        natural_midi = pitch_to_midi("#{letter}#{letter_octave}")
        accidental = midi - natural_midi
        accidental_text =
          case accidental
          when -2 then "bb"
          when -1 then "b"
        ...
        "#{letter}#{accidental_text}#{letter_octave}"
      end
